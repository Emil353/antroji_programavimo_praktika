...
namespace Antras_Praktinias_Darbas.Dialog {
    public partial class Dialog_Student : Window {
        SQL_Connection Conn;

        public Dialog_Student() {
            InitializeComponent();
        ...
        private void BTN_CreateNewUser(object sender, RoutedEventArgs e) {
            ...
            Conn.AddUser(TextBox_FirstName.Text, TextBox_LastName.Text, TextBox_FirstName.Text.ToLower(), TextBox_LastName.Text.ToLower(), 3);

            Close();

            _ = MessageBox.Show("New student has been created.", "Success!");
            Application.Current.Properties["users"] = Conn.GetUsers();

            ListBox Box = (ListBox)Application.Current.Properties["ListBox_Student"];
            Box.ItemsSource = Conn.GetStudents();
        }
    }
}
